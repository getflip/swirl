...
[the guide to creating new components](?path=/docs/contributions-create-a-component--page).
...
collisions, we also prefix our components with `flip-`.
...
component `flip-list` should start with the name of the parent component. In
this case `flip-list-item` would be appropriate.
...
Because of Stencil's scoping, we do not need prefixes for our class names.
