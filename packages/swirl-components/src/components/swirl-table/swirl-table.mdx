...
- `swirl-table-cell` `level` is **0-indexed**. Indent is
  `level * (space-24 + space-8)` (32px) **in addition to** the cell's 16px base
  inline padding.
- The 32px step is the plain toggle (24px) plus 8px gap before the label, so a
  leaf at level N lines up with its parent's label at level N-1.
