...
When `tree` and `expandable` are set, the toggle is a plain `swirl-button`.
Enter and Space activate it and emit `toggle`. There is 8px (`space-8`) between
the toggle and the slotted text.
