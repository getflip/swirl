import { Controls, Canvas, Meta, Story } from "@storybook/addon-docs/blocks";
import * as Stories from "./swirl-console-layout.stories";

<Meta title="Admin/SwirlConsoleLayout" />

# SwirlConsoleLayout

The SwirlConsoleLayout component provides the basic shell layout for the Flip
admin tools.

- **[Figma Design Specs](https://www.figma.com/file/Cks5hd1wSusGi1PYbpVgvX/Admindashboard?node-id=128%3A2201&t=RoIUBgbUhxFQOq8w-0)**
- **[Source Code](https://github.com/flip-corp/swirl/tree/main/packages/swirl-components/src/components/swirl-console-layout)**

## Usage

Please use the
[canvas view](?path=/story/admin-swirlconsolelayout--swirl-console-layout) to
preview this component.

<Controls of={Stories.SwirlConsoleLayout} />

## Related components

No related components.

## Accessibility

SwirlConsoleLayout is a layout shell with no ARIA attributes or keyboard-specific behavior. Ensure slotted navigation and content provide appropriate semantics and focus management.

### Keyboard

No component-specific keyboard behavior.
