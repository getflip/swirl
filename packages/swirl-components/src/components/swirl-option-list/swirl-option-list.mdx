...
<Canvas of={Stories.SwirlOptionList}></Canvas>

<Source
  code={`<swirl-option-list allow-drag label="Option List" multi-select value='["2"]'>
...
- **[swirl-option-list-section](?path=/docs/components-swirloptionlistsection--docs)**
  (child) — Optional grouping for options; use as a direct child of
  `swirl-option-list`.
- **[swirl-option-list-item](?path=/docs/components-swirloptionlistitem--docs)**
  (child) — Individual options; use as direct children of `swirl-option-list` or
  inside `swirl-option-list-section`. Often used inside
  [swirl-popover](?path=/docs/components-swirlpopover--docs) (e.g. with
