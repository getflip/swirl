...
describe("flip-option-list-item", () => {
  it("renders with label and icon", async () => {
    ...
    expect(page.root).toEqualHtml(`
      <flip-option-list-item context="single-select" disabled="false" icon="<flip-icon-mention></flip-icon-mention>" label="Option List Item" selected="true">
        <mock:shadow-root>
  ...
  it("renders for single-select context", async () => {
    const page = await newSpecPage({
      components: [FlipOptionListItem],
      html: `<flip-option-list-item
              label="Option List Item"
    ...
    expect(page.root.shadowRoot.children[0].getAttribute("aria-selected")).toBe(
      "true"
    );
  });

  it("renders for multi-select context", async () => {
    const page = await newSpecPage({
      components: [FlipOptionListItem],
      html: `<flip-option-list-item
              label="Option List Item"
    ...
    expect(page.root.shadowRoot.children[0].getAttribute("aria-checked")).toBe(
      "true"
    );
  });
});
