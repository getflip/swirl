...
[SwirlToast](?path=/docs/components-swirltoast--docs) messages.

- **[Source Code](https://github.com/flip-corp/swirl/tree/main/packages/swirl-components/src/components/swirl-toast-provider)**

## Usage

<Canvas of={Stories.SwirlToastProvider}></Canvas>

<Source
  code={`<swirl-toast-provider></swirl-toast-provider>`}
/>
...
- **[swirl-toast](?path=/docs/components-swirltoast--docs)** (child) — Toasts
  are created programmatically via the provider’s API; the provider renders
  `swirl-toast` instances.
