...
<Canvas of={Stories.SwirlTreeNavigationItem}></Canvas>

<Source
  code={`<swirl-tree-navigation-item icon="file" label="Label"></swirl-tree-navigation-item>`}
/>
...
- **[swirl-tree-navigation](?path=/docs/components-swirltreenavigation--docs)**
  (parent) — Navigation container; use `swirl-tree-navigation-item` as a direct
  child of `swirl-tree-navigation` or nested inside another item.
